...
while True:
    ...
    #Save ouput to files
    filesToMake = output
    with open((f"text0.txt"), "w") as file_object:
        file_object.write(filesToMake)
        if (f"text0.txt") not in filesMade:
            filesMade.append((f"text0.txt"))
...
if os.path.exists((f"text0.txt")):
    os.remove((f"text0.txt"))